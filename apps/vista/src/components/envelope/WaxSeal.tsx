...
export function WaxSeal({ onClick, isCracked = false }: WaxSealProps) {
  return (
    <motion.div
      className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-30 cursor-pointer group"
      onClick={onClick}
      animate={
        isCracked ? { scale: 1.1, opacity: 0 } : { scale: 1, opacity: 1 }
      }
    ...
    >
      <div className="relative h-24 w-24 sm:h-28 sm:w-28">
        ...
        <div className="absolute inset-0 rounded-full bg-[#861C22] opacity-0 blur-xl transition-opacity duration-500 group-hover:opacity-40" />

        {/* Outer shadow for depth */}
        <div className="absolute inset-0 rounded-full bg-[#0A0A08] opacity-40 blur-xl" />

        {/* Main seal body - dark red-brown */}
        <div className="absolute inset-1 rounded-full bg-linear-to-br from-[#671C1C] via-[#501515] to-[#2E0506] shadow-[0_4px_20px_rgba(0,0,0,0.7),inset_0_-2px_6px_rgba(0,0,0,0.4),inset_0_2px_4px_rgba(103,28,28,0.3)]" />
        ...
        <div className="absolute left-1/2 top-1/2 flex h-10 w-10 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-[#370D10] shadow-[inset_0_2px_4px_rgba(0,0,0,0.6)] sm:h-12 sm:w-12">
          ...
          <div
            className="flex items-center justify-center font-(family-name:--font-great-vibes) text-2xl text-[#D8CBBB] sm:text-3xl"
            style={{
              textShadow:
                "0 1px 0 rgba(0,0,0,0.8), 0 -1px 0 rgba(225,214,199,0.2), 0 0 6px rgba(225,214,199,0.3)",
